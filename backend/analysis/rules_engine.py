...
"""
PAESTRO - Módulo de Análise de Chamadas Escolares - Motor de Regras

Este módulo implementa as regras de classificação para determinar o status dos alunos
com base nos dados de frequência. Aplica regras diferentes dependendo do tipo de ensino
...
def apply_classification_rules(attendance_data):
    ...
    Regras para ensino não obrigatório (infantil, GT0-GT3):
    - Monitorar Faltas: 10+ faltas em um mês OU presença < 60%
    - Faltoso: 12+ faltas em um mês OU presença < 50%
    
    Regras para ensino obrigatório (fundamental, GT4-9º):
    - Monitorar Faltas: 7+ faltas em um mês OU presença < 60%
    - Faltoso: 10+ faltas em um mês OU presença < 50%
    ...
    Args:
        attendance_data (dict): Os dados de frequência analisados
        
    Returns:
        list: Lista de alunos com seus status de classificação
    """
    classified_students = []
    
    # Define níveis de educação para aplicação das regras
    ...
    compulsory = ['GT4', 'GT5'] + [f"{i}º" for i in range(1, 10)]
    ...
    for student in attendance_data.get('students', []):
        ...
        # Garantir que valores zero não sejam tratados como 'N/A'
        # Apenas verificar valores que são literalmente a string 'N/A'
        if (isinstance(total_present, str) and total_present == 'N/A') or \
           (isinstance(total_absences, str) and total_absences == 'N/A') or \
           (isinstance(total_justified, str) and total_justified == 'N/A'):
            # Se algum valor for a string 'N/A', convertemos para 0
            total_present = 0 if isinstance(total_present, str) and total_present == 'N/A' else total_present
            total_absences = 0 if isinstance(total_absences, str) and total_absences == 'N/A' else total_absences
            total_justified = 0 if isinstance(total_justified, str) and total_justified == 'N/A' else total_justified
            
        # Garantir que todos os valores são numéricos para soma
        # Tratar valores string e None convertendo para int
        try:
            total_present = 0 if total_present is None else int(total_present)
        except (ValueError, TypeError):
            total_present = 0
            
        try:
            total_absences = 0 if total_absences is None else int(total_absences)
        except (ValueError, TypeError):
            total_absences = 0
            
        try:
            total_justified = 0 if total_justified is None else int(total_justified)
        except (ValueError, TypeError):
            total_justified = 0
        
        # Calcular total de registros com valores numéricos garantidos
        ...
        # Verifica se o aluno já tem a informação de obrigatoriedade
        # passada pelo novo parser (campo is_compulsory)
        if 'is_compulsory' in student and student['is_compulsory'] is not None:
            education_type = 'compulsory' if student['is_compulsory'] else 'non_compulsory'
        else:
            # Compatibilidade com dados antigos: determina com base no nome da turma
            education_type = None
            
            # Primeiro verificamos por GT0, GT1, GT2, GT3 (ensino não obrigatório)
            # Verificação para detectar GT3 em diferentes formatos (GT3A, GT 3 A, etc)
            ...
            # Verificação adicional pelos campos de tipo de educação
            if not education_type and 'education_type' in student:
                parser_type = student.get('education_type', '').lower()
                if parser_type == 'obrigatorio' or parser_type == 'fundamental':
                    education_type = 'compulsory'
                elif parser_type == 'infantil' and 'GT4' in class_name or 'GT5' in class_name:
                    # Para GT4 e GT5 que são infantil mas obrigatório
                    education_type = 'compulsory'
                elif parser_type == 'infantil':
                    # Outros casos de infantil são não obrigatórios 
                    education_type = 'non_compulsory'
        ...
        # Attendance percentage threshold for absentee (menos de 60% de presença)
        if total_records > 0 and attendance_percentage != 'N/A' and attendance_percentage < 60:
            is_absentee = True
        ...
        if "Faltoso" not in statuses:
            ...
            # Attendance percentage threshold for monitoring (menos de 75% de presença)
            if total_records > 0 and attendance_percentage != 'N/A' and attendance_percentage < 75:
                monitor_absences = True
        ...
        if total_records > 0 and total_justified != 'N/A' and total_justified > 0:
